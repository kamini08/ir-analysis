# IR Lifting Benchmark PoC (Ghidra vs. angr)

## 1. Project overview
Benchmarks IR lifting performance of Ghidra (P-code), angr (VEX), and optional LLVM IR pipelines on small benign binaries to compare runtime and memory consumption for security research workflows.
This README aligns with the accompanying comparative analysis of IR design trade-offs (P-code, VEX, LLVM IR) and helps map empirical benchmarks to the paper’s discussion of vulnerability discovery, malware triage, and deobfuscation tasks.
...
| Path | Purpose |
| --- | --- |
| `scripts/` | Automation scripts (`analyze_ghidra.sh`, `analyze_angr.py`, `run_all.sh`), plus any custom adapters you add for additional IR toolchains (e.g., LLVM) |
...
4. (Optional) Install LLVM 15+ binaries if you plan to benchmark an LLVM IR pipeline:  
   `sudo apt install llvm`
5. Download Ghidra and note its install path (e.g., `/opt/ghidra`).
6. Place benign samples in `samples/benign/`. Do **not** add malware to git.
7. Configure script paths (see below) and run `./scripts/run_all.sh`.
...
```
The wrapper:
- Launches the Ghidra and angr analysis helpers (and any optional LLVM IR adapter you hook into `run_all.sh`).
- Wraps each run with `time -v`.
- Writes raw logs plus parsed metrics to `results/`.

## 9. Understanding results
- `results/analysis_results.log` — mixed raw + human-readable summary produced per run. Expect one block per configured tool (Ghidra / angr / LLVM IR).  
  Look for:
...
- [ ] Parsed metrics show entries for every enabled tool (Ghidra, angr, and LLVM IR if configured).
